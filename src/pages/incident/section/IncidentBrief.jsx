...
import IncidentBriefCard from "./IncidentBriefCard";
...
const IncidentBrief = ({ brief = [], onAddReport }) => {
  return (
    <div className="flex flex-col h-auto">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 p-4 bg-white sticky top-0 z-10">
        ...
        >
          <FiPlusCircle /> Add Brief
        </button>
      </div>

      {/* Reports List */}
      <div className="flex-1 overflow-y-auto p-2 tab-scroll flex justify-start bg-gray-50">
        {brief.length > 0 ? (
       ...
       {sampleReports.map((report, index) => {
        return (
          <li key={index}>
            <IncidentBriefCard report={report} />
          </li>
        )
